...
class JournalEntries extends Table {
  ...
  DateTimeColumn get date =>
      dateTime()(); // Date of the journal entry (without time)
  ...
  TextColumn get originalAiSummary =>
      text().nullable()(); // Original AI summary (for edit detection)
  TextColumn get summaryHash =>
      text().nullable()(); // SHA-256 hash of original AI summary
  BoolColumn get summaryWasEdited =>
      boolean().withDefault(const Constant(false))(); // User edited summary
  BoolColumn get isAutoGenerated =>
      boolean().withDefault(const Constant(true))(); // Auto vs user created
  BoolColumn get isEdited =>
      boolean().withDefault(const Constant(false))(); // User has edited
...
class JournalActivities extends Table {
  ...
  TextColumn get activityType =>
      text()(); // 'location', 'photo', 'movement', 'calendar', 'manual'
  TextColumn get description => text()();
  TextColumn get metadata =>
      text().nullable()(); // JSON with activity-specific data
...
@DriftDatabase(
  tables: [JournalEntries, JournalActivities, JournalTemplates],
...
class JournalDatabase extends _$JournalDatabase {
  ...
  Future<bool> updateJournalEntry(JournalEntriesCompanion entry) async {
    final updatedRows = await (update(
      journalEntries,
    )..where((tbl) => tbl.id.equals(entry.id.value))).write(entry);
  ...
  Stream<JournalEntry?> watchJournalEntryForDate(DateTime date) {
    ...
    return (select(
      journalEntries,
    )..where((tbl) => tbl.date.equals(dateOnly))).watchSingleOrNull();
  }

  Stream<List<JournalEntry>> watchRecentJournalEntries({int limit = 30}) {
  ...
  Future<List<JournalEntry>> searchJournalEntriesFts(
    String query, {
    int limit = 50,
  }) async {
  ...
  Future<int> deleteActivitiesForEntry(int journalEntryId) {
    return (delete(
      journalActivities,
    )..where((tbl) => tbl.journalEntryId.equals(journalEntryId))).go();
  }
  ...
  Future<Map<String, int>> getJournalStatistics() async {
    final totalEntries = await (selectOnly(
      journalEntries,
    )..addColumns([journalEntries.id.count()])).getSingle();

    final autoGenerated =
        await (selectOnly(journalEntries)
  ...
  Future<void> cleanupOldEntries({
    ...
    await (delete(
      journalEntries,
    )..where((tbl) => tbl.date.isSmallerThanValue(cutoff))).go();
  }
  ...
  MigrationStrategy get migration => MigrationStrategy(
    beforeOpen: (details) async {
      ...
      if (details.versionBefore != null &&
        ...
        debugPrint(
          'Journal database migration: v${details.versionBefore} -> v${details.versionNow}',
        );
      }
    },
    ...
    onUpgrade: (Migrator m, int from, int to) async {
      if (from < 2) {
        ...
        try {
          ...
          try {
            ...
            debugPrint(
              'Note: Could not create search triggers (may already exist): $e',
            );
          }
          ...
          try {
            ...
            debugPrint(
              'Warning: Could not populate FTS5 index during migration: $e',
            );
          }
        } catch (e) {
          // Log the error but don't crash the app
          debugPrint(
            'Error during journal database migration from v$from to v$to: $e',
          );
      ...
      if (from < 3) {
        ...
        try {
          ...
          debugPrint(
            'Note: Could not add original_ai_summary column (may already exist): $e',
          );
        }
        ...
        try {
          ...
          debugPrint(
            'Note: Could not add summary_hash column (may already exist): $e',
          );
        }
        ...
        try {
          ...
          debugPrint(
            'Note: Could not add summary_was_edited column (may already exist): $e',
          );
        }

        debugPrint(
          'Journal database migrated to v3: Added user edit protection for summaries',
        );
      }
    },
  );
