...
class JournalService {
  ...
  final ai_service.AIService _aiService;
  ...
  Future<JournalEntry> createEntryForDate(DateTime date) async {
    ...
    try {
      ...
      final contentLines = aiEntry.content.split('\n');
      ...
      // Determine mood from content (simple heuristic)
      final mood = _determineMoodFromContent(aiEntry.content);

      // Extract tags from content and metadata
      final tags = _extractTagsFromContent(aiEntry.content, aiEntry.metadata);
      ...
      final entryId = await _journalDb.insertJournalEntry(
        JournalEntriesCompanion(
          ...
          content: Value(aiEntry.content),
          mood: Value(mood),
          tags: Value(jsonEncode(tags)),
          summary: Value(aiEntry.summary),
      ...
      await _journalDb.insertJournalEntry(
        JournalEntriesCompanion(
          ...
          mood: const Value('neutral'),
          tags: Value(jsonEncode(['daily', 'reflection'])),
  ...
  /// Determine mood from content using simple heuristics
  String _determineMoodFromContent(String content) {
    final lowerContent = content.toLowerCase();

    // Positive mood indicators
    final positiveWords = ['happy', 'joy', 'great', 'amazing', 'wonderful', 'excited', 'grateful', 'love', 'perfect', 'fantastic'];
    final negativeWords = ['sad', 'tired', 'difficult', 'hard', 'stressed', 'worried', 'anxious', 'frustrated', 'upset'];
    final calmWords = ['peaceful', 'calm', 'relaxed', 'balanced', 'serene', 'meditation', 'quiet'];
    ...
    for (final word in calmWords) {
      if (lowerContent.contains(word)) calmCount++;
    }

    if (calmCount > 0 && calmCount >= positiveCount && calmCount >= negativeCount) {
      return 'peaceful';
  ...
  /// Extract tags from content and metadata
  List<String> _extractTagsFromContent(String content, Map<String, dynamic> metadata) {
    final tags = <String>[];
    ...
    if (metadata.containsKey('tags')) {
      ...
      if (metaTags is List) {
        tags.addAll(metaTags.cast<String>());
      }
    }
    ...
    if (lowerContent.contains('family') || lowerContent.contains('friend') || lowerContent.contains('social')) {
      tags.add('social');
    }
    if (lowerContent.contains('exercise') || lowerContent.contains('walk') || lowerContent.contains('run') || lowerContent.contains('gym')) {
      tags.add('fitness');
    }
    if (lowerContent.contains('food') || lowerContent.contains('cook') || lowerContent.contains('meal') || lowerContent.contains('eat')) {
      tags.add('food');
    }
    if (lowerContent.contains('travel') || lowerContent.contains('trip') || lowerContent.contains('visit')) {
      tags.add('travel');
    }
    if (lowerContent.contains('learn') || lowerContent.contains('read') || lowerContent.contains('study')) {
      tags.add('learning');
    }
    if (lowerContent.contains('creative') || lowerContent.contains('art') || lowerContent.contains('music') || lowerContent.contains('draw')) {
      tags.add('creative');
    }
