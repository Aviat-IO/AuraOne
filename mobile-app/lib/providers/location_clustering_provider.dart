...
class _ClusteringCache {
  ...
  _CacheEntry? get(String key) {
    ...
    if (entry != null &&
        DateTime.now().difference(entry.timestamp) < cacheExpiry) {
      return entry;
    }
    cache.remove(key);
    return null;
  ...
  void invalidateDay(String dayKey) {
    cache.removeWhere((key, _) => key.contains('_${dayKey}_'));
  }
  ...
  void _cleanupOldEntries() {
    ...
    _logger.info(
      'Cleaned up $toRemove old cache entries due to memory pressure',
    );
  }
}
...
int _watchDayInvalidation(Ref ref, DateTime date) {
  final dayKey = locationCacheDayKey(date);
  ref.watch(locationWriteInvalidationBindingProvider);
  ref.listen<int>(
    locationDayInvalidationProvider.select((versions) => versions[dayKey] ?? 0),
    (previous, next) {
      if (previous != null && next > previous) {
        _clusteringCache.invalidateDay(dayKey);
      }
    },
  );
  return ref.watch(
...
int _watchGlobalInvalidation(Ref ref) {
  ref.listen<int>(
...
final clusteredLocationsProvider = FutureProvider.family<List<LocationCluster>, DateTime>((
  ...
  final invalidationVersion = _watchDayInvalidation(ref, date);
  final globalInvalidationVersion = _watchGlobalInvalidation(ref);
  ...
  try {
    ...
    final cacheKey = [
      'clusters',
      dayKey,
      'v$invalidationVersion',
      'g$globalInvalidationVersion',
    ].join('_');
    ...
    final locationHistory = await ref
    ...
    _logger.info(
      'Loaded ${locationHistory.length} raw location points for date $date',
    );
    ...
    if (dayLocations.length > 1000) {
      ...
      _logger.info(
        'Limited location points to 1000 most recent for performance',
      );
    }

    _logger.info(
      'Loaded ${locationHistory.length} points for clustering, filtered to ${dayLocations.length} for date ${date.year}-${date.month}-${date.day}',
    );
    ...
    if (dayLocations.length > 2000) {
      _logger.warning(
        'Too many location points (${dayLocations.length}) for date $date, using simplified clustering',
      );
    ...
    try {
      filteredLocations = await Future.microtask(
        () => _removeGeographicOutliers(filteredLocations),
    ...
    try {
      filteredLocations = await Future.microtask(
        () => _removeSpeedOutliers(filteredLocations),
      ).timeout(const Duration(seconds: 2));
    } catch (e) {
    ...
    try {
      filteredLocations = await Future.microtask(
        () => _removeConsecutiveDistanceOutliers(filteredLocations),
      ).timeout(const Duration(seconds: 2));
    } catch (e) {
      _logger.warning(
        'Consecutive distance outlier filtering timed out or failed: $e',
      );
    }
    ...
    try {
      result =
          await HybridClustering.smartCluster(
            clusteringPoints,
            eps: 150.0,
            minPts: 2,
          ).timeout(
      ...
      try {
        result = await Future.microtask(
          () => _simpleClusteringFallback(clusteringPoints),
    ...
    try {
      significantClusters = await Future.microtask(
        () => ClusterMerger.smartMerge(
          significantClusters,
          locationRadius: 150,
...
Future<List<LocationCluster>> _fastClusterLargeDataset(
  ...
  for (int i = 1; i < sortedLocations.length; i++) {
    ...
    if (current.timestamp.difference(lastInGroup.timestamp) <=
        intervalDuration) {
...
final journeySegmentsProvider = FutureProvider.family<List<JourneySegment>, DateTime>((
  ...
  try {
    ...
    final cachedEntry = _clusteringCache.get(
      [
        'clusters',
        dayKey,
        'v$invalidationVersion',
    ...
    final locationHistory = await ref.read(
      locationPointsForDateProvider(date).future,
    );
    ...
    _logger.info(
      'Loaded ${locationHistory.length} points for journeys, filtered to ${dayLocations.length} for date ${date.year}-${date.month}-${date.day}',
    );
...
List<loc_db.LocationPoint> _removeGeographicOutliers(
  List<loc_db.LocationPoint> locations,
) {
  ...
  // Calculate MAD
  final deviations = distances.map((d) => (d - medianDistance).abs()).toList()
    ..sort();
  ...
  for (int i = 0; i < locations.length; i++) {
    if (distances[i] <= threshold) {
      ...
      _logger.info(
        'Filtered outlier at ${locations[i].latitude}, ${locations[i].longitude} (distance from median: ${distances[i].toStringAsFixed(0)}m)',
      );
    }
  }
...
List<loc_db.LocationPoint> _removeSpeedOutliers(
  List<loc_db.LocationPoint> locations,
) {
  ...
  for (int i = 1; i < sorted.length; i++) {
    final prev = filtered
        .last; // Compare against last filtered point, not previous raw point
    final curr = sorted[i];
    ...
    if (timeDiff > 0) {
      ...
      if (speed <= maxSpeedMetersPerSecond) {
        ...
        _logger.info(
          'Filtered speed outlier: ${speed.toStringAsFixed(1)} m/s (${(speed * 3.6).toStringAsFixed(1)} km/h) between ${prev.timestamp} and ${curr.timestamp}',
        );
      }
    } else {
...
List<loc_db.LocationPoint> _removeConsecutiveDistanceOutliers(
  List<loc_db.LocationPoint> locations,
) {
  ...
  final defaultThreshold =
      150.0; // Default for unknown activities (very conservative)
  ...
  final finalThreshold = madThreshold < maxExpectedDistance
      ? madThreshold
      : maxExpectedDistance;
  ...
  for (int i = 1; i < sorted.length; i++) {
    ...
    if (distance <= finalThreshold) {
      ...
      _logger.info(
        'Filtered consecutive distance outlier: ${distance.toStringAsFixed(1)}m (threshold: ${finalThreshold.toStringAsFixed(1)}m, activity: ${activityTypes[i - 1] ?? "unknown"})',
      );
    }
  }
